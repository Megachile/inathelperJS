...
### Changed
- Moved the Bulk Action Records retention explanation into an accessible info tooltip beside the modal title, keeping the record list compact while preserving the warning on hover and keyboard focus.
...
### Changed
- **"Close and Refresh" now waits for iNaturalist to actually catch up** (#64, reported on the forum) — the button reloaded immediately, before iNaturalist had finished propagating the bulk edits to the search index that repopulates the observations grid. The page came back showing the same data, which is indistinguishable from the button doing nothing; the automatic post-action refresh had a deliberate 2 second wait and didn't have the problem. Rather than give the button the same fixed delay, both paths now wait for the grid itself: iNaturalist's search index lags its own writes by a variable amount, and on a 50-observation batch rows kept dropping out of the filter anywhere from 0.6 to 8.8 seconds after the edits landed, so no fixed wait is right for everyone. The extension now polls the page's own filter for the observations it just touched and reloads once the count stops moving — or immediately, confirmed once, when nothing you edited still matches. The button shows a "Refreshing…" state with progress while it waits, there's a 15 second cap, and pages with no filter to watch fall back to a flat wait. Both paths read the same shared code so they can't drift apart again.
...
### Fixed
- **"Enable Bulk Action Mode" button went off-screen when cycling button positions** (#56, reported by @rcavasin on Firefox) — the button was only visible when the main buttons sat at the default bottom-right corner and disappeared at every other Alt+N position. When the `#bulk-ui-wrapper` was introduced to hold all bulk UI, the `#enable-bulk-mode-button` CSS kept its own stale `position: fixed` (with no offsets), so a child `position: fixed` element anchored to the viewport instead of following its wrapper. Removed the rule so the button flows normally inside the fixed wrapper and follows it to whichever corner it moves to.
- **Clicking a taxon autocomplete suggestion could fail to register** — the suggestion list is hidden ~200 ms after the input blurs, and the selection ran on `click`, which fires after blur; in some cases the list was torn down before the click landed. Selection now runs on `mousedown` with `preventDefault`, which fires before blur and keeps the input focused, so picking a suggestion is reliable. The suggestion markup was also simplified (common name shown inline, with the taxon page moved to a small `↗` link).

### Note
- #56 also includes a possible Mac-only Ctrl/Cmd+click report; that part is deferred pending confirmation from the reporter, so no behavior change there yet.
